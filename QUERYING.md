...
up. The [README](README.md) covers what each group *is* and how it was gathered;
...
|---|---|
| `data/catalog.json` | **The index.** One entry per *table*, with `grain` (what one row is), `columns`, and `joins`. |
| `data/manifest.json` | The same, per *file*: byte size, source URL, record count, run status, last refresh. |
...
for name, group in catalog["groups"].items():
    for t in group["tables"]:
        if t.get("grain"):
            print(t["path"], "->", t["grain"])
```

28 of 133 tables carry a `grain`/`joins` description; the rest appear without
...
- Files beginning with `_` are *about* the data, not data.
...
*second, different* course table — see [the two course tables](#the-two-course-tables).

| Table | Rows | One row is |
|---|---|---|
| `courses` | 9,491 | A course *description* with prerequisites parsed out of the prose |
...
| `ubcv_building_entraces` | 1,474 | Point — *(upstream typo in the filename, kept as-is)* |
| `ubcv_address` | 1,230 | Point |
| `ubcv_poi` | 489 | Point — cafés, restaurants, libraries, services, with `HOURS` |
| | | *(the CSV twins of these carry `LAT`/`LONG`; see [coordinate columns](#coordinate-columns-fixed-here-wrong-upstream))* |
...
| `requirements/program_requirements` | 141 | program → requirement group *(the first hop of every requirements query)* |
...
```python
courses  = {c["id"]: c for c in json.load(open("data/courses/courses.json", encoding="utf-8"))}
sections = json.load(open("data/courses/sections.json", encoding="utf-8"))

for s in sections[:5]:
    course = courses[s["related"]["course"]]
    print(course["field_course_code"], s["field_days"], s["field_instructors"])
```
...
⚠️ **`location_slug` is not unique on its own.** `basic` exists in *both* the
...
*offered*; the calendar says what a course *is* and what it requires.

They do not share a key. Build one:

```python
subjects = {s["id"]: s["name"]
            for s in json.load(open("data/academic-calendar/vancouver/subjects.json", encoding="utf-8"))}

def calendar_code(row):
    """'AANB_V 550' -- matches courses/courses.field_course_code."""
    name = subjects.get(row["related"].get("course_code"))
    number = row.get("field_course_number")
    return f"{name} {number}" if name and number is not None else None
```
...
```python
h, m = divmod(section["field_start_time"] // 60, 60)
...
may not be now. Re-run `python update.py bookings` (~5s) before relying on it.
...
```python
import json
L = lambda p: json.load(open(f"data/{p}", encoding="utf-8"))

availability = L("room-bookings/availability.json")
geo          = L("geospatial/ubcv/locations/geojson/ubcv_buildings.geojson")

where = {}
for f in geo["features"]:
    code = f["properties"].get("BLDG_CODE")
    coords = f["geometry"]["coordinates"]
    while isinstance(coords[0], list):
        coords = coords[0]
    where.setdefault(code, {"name": f["properties"]["NAME"],
                            "lat": coords[1], "lon": coords[0]})

hits = [a for a in availability
        if a["state"] == "free" and a["minutes"] >= 120 and (a["capacity"] or 0) >= 8]

for a in sorted(hits, key=lambda x: -x["capacity"])[:5]:
    b = where[a["building_code"]]
    print(f"{a['room']:30} {a['capacity']:>3} seats  {a['start'][11:16]}-{a['end'][11:16]}")
    print(f"   {b['name']}  {b['lat']:.5f},{b['lon']:.5f}")
```

### What does Engineering require, from BC?

```python
req = L("admissions/requirements/required_courses.json")
rows = [r for r in req
        if r["requirement_key"] == "engineering"
        and r["location_slug"] == "british-columbia"
        and not r["advisory"]]
```
...
```python
courses  = {c["id"]: c for c in L("courses/courses.json")}
sections = L("courses/sections.json")

DAYS = {"m": "Mon", "t": "Tue", "w": "Wed", "th": "Thu", "f": "Fri"}

for s in sections:
    c = courses.get(s["related"].get("course"))
    if c and c["field_course_code"] == "CPSC_V 110" and s["field_start_time"]:
        days = " ".join(DAYS[d] for d in s["field_days"])
        h, m = divmod(s["field_start_time"] // 60, 60)
        print(f"{s['field_section_number']:6} {days:16} {h:02d}:{m:02d}  {s['field_instructors']}")
```

### Cafés open near a point

```python
poi = L("geospatial/ubcv/locations/geojson/ubcv_poi.geojson")
cafes = [f for f in poi["features"]
         if f["properties"]["SERVICE_TYPE"] in ("cafe", "restaurant")]
...
```python
est = L("finances/program_cost_estimates.json")
row = next(r for r in est if r["program"].startswith("Computer Science"))
print(row["educational_total_domestic"], row["educational_total_international"])
print("matched to estimator area by:", row["matched_by"])
```
...
| **Where is Professor Y right now?** | 494/1,718 profiles have an office string, in no consistent format; office *hours* aren't in the data at all. |
...
python update.py bookings           # just availability (~5s, snapshot — re-run often)
