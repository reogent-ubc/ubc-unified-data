...
regenerated with per-file record counts, sizes, sources and timestamps, and
...
`data/manifest.json` is the same information plus per-file byte sizes, source
URLs, timestamps and run status. Collectors declare grain and joins with
`out.describe(...)`; datasets that haven't been described yet still appear, just
...
- **Requirements are shared between programs.** All 41 Arts majors ask the same
  ...
  every query.
- **19 programs have no direct-entry rules at all** — Medicine, Law, Dentistry,
...
- **Floor is worked out, not fetched.** Airtable has no floor field. Informal
  spaces usually say it in words ("B Block 2nd Floor Lounge"); classrooms encode
  it as the leading digit of the room number, behind an optional block letter —
  `BUCH B101` is Block B, room 101, floor 1, not a basement. `floor_source` says
  which route was taken. 350 of 411 rooms resolve; the rest are **left null
  rather than guessed**, because IRC numbers its rooms 1–6, G41–G66 and B75–B79
  on a scheme of its own, FNH uses 30/40/50/60, and a wrong floor is worse to a
  student looking for a room than no floor.
- **Photo URLs expire.** UBC serves images from Airtable behind signed links
  ...
  `reports` aren't: this indexes what UBC publishes, it doesn't rehost it.
  `Room Link` is the durable route — it re-renders a fresh URL on load — while
  `cover_photo_expires` gives the deadline on the stored one and
  `cover_photo_id` outlives it, so a stale row reads as stale instead of just
  broken. 409 of 411 rooms have a photo (`BUCH B202` and MCLD's catch-all row
  have none); each building takes its largest room's.

`buildings` used to hold 38 rows because the buildings endpoint answers for
whichever tab it is asked about and was only asked once — the 12 buildings that
hold classrooms but no informal space were missing. It is asked per tab now, so
all 50 are there and every room joins to one.
...
instalment counts — go to `fee_notes` rather than being dropped.
...
Architecture are graduate; "Bachelor of Education" genuinely spans two areas)
are in `_unmatched.json` rather than being given a wrong price.
...
- **The calendar is more than courses.** Degree and specialization requirements,
  ...
  `kind` (`degree_requirements`, `major`, `minor`, `admission`, `regulations`…)
  from that chain plus the title. `referenced_courses` lists the course codes a
